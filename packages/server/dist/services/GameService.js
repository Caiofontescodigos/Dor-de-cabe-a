export class GameService {
    ...
    // 🎯 PLAY MOVE
    // ===============================
    playMove(game, playerId, dominoId, side) {
    ...
    // 🃏 DRAW
    // ===============================
    drawUntilPlayable(game, playerId) {
        if (game.status !== 'playing')
            return null;
        if (game.currentPlayer !== playerId)
            return null;
    ...
    calculateScores(game) {
        ...
        for (const p of game.players) {
            ...
            const total = hand.reduce((sum, d) => sum + d.left + d.right, 0);
            scores.set(p, total);
        }
        return scores;
    }
    ...
    getValidMoves(hand, board) {
        ...
        return hand.filter((d) => d.left === left ||
            d.right === left ||
            d.left === right ||
            d.right === right);
    }
    ...
    // 🧠 START PLAYER
    // ===============================
    findStartingPlayer(hands, players) {
        let best = players[0];
        let bestVal = -1;
        for (const p of players) {
            const hand = hands.get(p);
            for (const d of hand) {
                if (d.left === 6 && d.right === 6)
                    return p;
